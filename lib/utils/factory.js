class DO_NOT_TOUCH {
  ...
  static param = /@wpiParam\s(\{(?<type>.+)\})\s(?<key>[a-zA-Z0-9]+)\s(?<explain>[^\s].*)/;
  static query = /@wpiQuery\s(\{(?<type>.+)\})\s(?<key>[a-zA-Z0-9]+)\s(?<explain>[^\s].*)/;
  static success = /@wpiSuccess\s(\{(?<type>.+)\})\s(?<key>[a-zA-Z0-9]+)\s(?<explain>[^\s].*)/;
  static fail = /@wpiFail\s(\{(?<type>.+)\})\s(?<key>[a-zA-Z0-9]+)\s(?<explain>[^\s].*)/;
}

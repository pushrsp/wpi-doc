/**
 * @wpiDefineKey hello
 * @wpiDefineValue {string} test 테스트
 * @wpiDefineValue {int} hi 테스트
 * @wpiDefineValue {string} bye 테스트
 * @wpiDefineValue {ko} ko 재귀
 */

/**
 * @wpiDefineKey react
 * @wpiDefineValue {string} vue 뷰
 * @wpiDefineValue {string} svelte 스벨트
 */

/**
 * @wpiDefineKey ko
 * @wpiDefineValue {string} ok 재귀테스트
 * @wpiDefineValue {int} code 코드
 * @wpiDefineValue {react} web 웹프레임워크
 */

/**
 * @wpiDefineKey bye
 * @wpiDefineValue {string} ollah 바이
 */

/**
 * @wpiDefineKey hi
 * @wpiDefineValue {string} abc 하이
 */

/**
 * @wpiRoute api/or [post]
 * @wpiDescription 'or' 테스트입니다.
 * @wpiBody {hello | bye} bye or
 */
